 ...
 */
class BackgroundUpdater extends WP_Background_Process {
	/**
	...
	public function dispatch() {
		...
		if ( is_wp_error( $dispatched ) ) {
			Compatibility::log( sprintf( 'Unable to dispatch WooCommerce Composite Products updater: %s', $dispatched->get_error_message() ), 'error', 'wc_cp_db_updates' );
		}
	}
	...
	protected function task( $callback ) {
		...
		if ( is_callable( $callback ) ) {
			...
			Compatibility::log( sprintf( '- Running %s callback...', $callback ), 'info', 'wc_cp_db_updates' );
			...
			Compatibility::log( $message, 'info', 'wc_cp_db_updates' );

		} else {

			$result = false;

			Compatibility::log( sprintf( '- Could not find %s callback.', $callback ), 'notice', 'wc_cp_db_updates' );
		}
