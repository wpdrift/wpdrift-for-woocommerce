...
class Product {
	 ...
	 * @param  ProductComposite  $parent
	 */
	public function __construct( $product_id, $component_id, $parent ) {

		$product_id = absint( $product_id );

		if ( $product_id > 0 && ( $product = wc_get_product( $product_id ) ) && in_array( $product->get_type(), ProductComposite::get_supported_component_option_types( true ) ) ) {
		...
		if ( $this->is_priced_individually() && $this->is_purchasable() ) {
			...
			if ( in_array( $product_type, array( 'simple', 'variation' ) ) ) {
				...
				if ( $discount = $this->get_discount() ) {
					$this->min_price = empty( $min_regular_price ) ? $min_regular_price : round( (float) $min_regular_price * ( 100 - $discount ) / 100, wc_cp_price_num_decimals() );
					$this->max_price = empty( $max_regular_price ) ? $max_regular_price : round( (float) $max_regular_price * ( 100 - $discount ) / 100, wc_cp_price_num_decimals() );
				} else {
	...
	public function is_priced_individually() {
		 ...
		 * @param  Product  $composited_product
		 */
	...
	public function has_addons( $required = false ) {
		...
		// Any add-ons?
		if ( Module::instance()->compatibility->has_addons( $this->get_product(), $required ) ) {
			$has_addons = true;
		}
	...
	public function get_raw_price( $product = false, $context = '' ) {
		...
		if ( $discount = $this->get_discount() ) {
			$price = empty( $regular_price ) ? $regular_price : round( (float) $regular_price * ( 100 - $discount ) / 100, wc_cp_price_num_decimals() );
		}
	...
	public function calculate_price( $args ) {
		...
		return apply_filters(
			'woocommerce_get_composited_product_price',
	 ...
	 * @see 'ComponentView::get_options_data'.
	 *
	 * @return array|false
	 */
	...
	public function get_price_string() {
		...
		if ( $this->is_priced_individually() && $this->is_purchasable() ) {
			...
			if ( $ref_price > $price ) {
				...
				if ( $discount_amount ) {
					$discount = sprintf( __( '(%s%% off)', 'wpdrift-woocommerce-modules' ), round( $discount_amount, 1 ) );
				}

				if ( ! $discount && $ref_price > $price ) {
					$sale = sprintf( __( '(%s%% off)', 'wpdrift-woocommerce-modules' ), round( 100 * ( $ref_price - $price ) / $ref_price, 1 ) );
				}
			}
			...
			$price_string = Helpers::format_raw_price( $price );
			$qty_suffix   = $has_multiple ? __( '/ pc.', 'wpdrift-woocommerce-modules' ) : '';
			 ...
			 * @param  Product  $composited_product
			 */
			$price_string = apply_filters( 'woocommerce_composited_product_price_string_inner', sprintf( _x( '%1$s %2$s %3$s', 'option price followed by per unit suffix and discount', 'wpdrift-woocommerce-modules' ), $price_string, $qty_suffix, $suffix ), $price_string, $qty_suffix, $suffix, $price, $is_range, $has_multiple, $product_id, $component_id, $this );
			$price_string = $is_range ? sprintf( _x( 'from %s', 'Price range - from', 'wpdrift-woocommerce-modules' ), $price_string ) : $price_string;
		}
	...
	public static function get_title_string( $title, $qty = '', $price = '' ) {
		...
		if ( $qty ) {
			$quantity_string = sprintf( _x( ' &times; %s', 'qty string', 'wpdrift-woocommerce-modules' ), $qty );
		}

		if ( $price ) {
			$price_string = sprintf( _x( ' &ndash; %s', 'price suffix', 'wpdrift-woocommerce-modules' ), $price );
		}

		$title_string = sprintf( _x( '%1$s%2$s%3$s', 'title quantity price', 'wpdrift-woocommerce-modules' ), $title, $quantity_string, $price_string );
	...
	public function get_availability( $product = false ) {
		 ...
		 */
		return apply_filters(
			'woocommerce_composited_product_availability',
			array(
	...
	private function get_availability_text( $product = false ) {
		...
		if ( ! $product->is_in_stock() ) {
			...
			$availability = __( 'Out of stock', 'wpdrift-woocommerce-modules' );
			...
			if ( $product->backorders_require_notification() ) {
				...
				switch ( get_option( 'woocommerce_stock_format' ) ) {
					...
					case 'no_amount':
						$availability = __( 'Available on backorder', 'wpdrift-woocommerce-modules' );
						break;
					default:
						$availability = __( 'Available on backorder', 'wpdrift-woocommerce-modules' );

						if ( $total_stock > 0 ) {
							$availability .= ' ' . sprintf( __( '(only %s left in stock)', 'wpdrift-woocommerce-modules' ), $total_stock );
						}

						break;
				}
			} else {
				$availability = __( 'In stock', 'wpdrift-woocommerce-modules' );
			}
			...
			if ( $total_stock >= $quantity ) {
					...
					case 'low_amount':
						if ( $total_stock <= get_option( 'woocommerce_notify_low_stock_amount' ) ) {
							...
							if ( $product->backorders_allowed() && $product->backorders_require_notification() ) {
								$availability .= ' ' . __( '(also available on backorder)', 'wpdrift-woocommerce-modules' );
							}
						} else {
							$availability = __( 'In stock', 'wpdrift-woocommerce-modules' );
						}
						...
						break;
					default:
						$availability = sprintf( __( '%s in stock', 'wpdrift-woocommerce-modules' ), $total_stock );
				...
				switch ( get_option( 'woocommerce_stock_format' ) ) {
					...
					case 'no_amount':
						$availability = __( 'Insufficient stock', 'wpdrift-woocommerce-modules' );
						break;
					default:
						$availability  = __( 'Insufficient stock', 'wpdrift-woocommerce-modules' );
						$availability .= ' ' . sprintf( __( '(only %s left in stock)', 'wpdrift-woocommerce-modules' ), $total_stock );
			...
			if ( class_exists( 'Admin_Ajax' ) && Admin_Ajax::is_composite_edit_request() ) {
				$availability = __( 'In stock', 'wpdrift-woocommerce-modules' );
			}
		}
	...
	public function get_image_size() {
		return apply_filters( 'woocommerce_composite_component_option_image_size', Compatibility::is_wc_version_gte( '3.3' ) ? 'woocommerce_thumbnail' : 'shop_catalog', $this );
	}
	...
	public function get_selection_thumbnail_size() {
		return apply_filters( 'woocommerce_composited_product_thumbnail_size', Compatibility::is_wc_version_gte( '3.3' ) ? 'woocommerce_thumbnail' : 'shop_catalog', $this );
	}
	...
	public function get_tax_ratios() {
		return Products::get_tax_ratios( $this->get_product() );
	}
	...
	*/
	...
	public static function get_placeholder_product_data( $context = 'no-product', $args = array() ) {
		...
		wc_get_template( 'composited-product/' . $context . '.php', $args, '', Module::instance()->plugin_path() . '/templates/' );
