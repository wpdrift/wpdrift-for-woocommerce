...
class Products {
	...
	*/
	...
	public static function initialize_price_calc_task_runner() {
		self::$price_calc_task_runner = new PriceCalcTaskRunner();
	}
	...
	public static function get_tax_ratios( $product ) {
		Helpers::extend_price_display_precision();
		...
		Helpers::reset_price_display_precision();
	...
	public static function get_discounted_price( $price, $discount ) {
		...
		if ( ! empty( $price ) && ! empty( $discount ) ) {
			$discounted_price = round( (float) $price * ( 100 - $discount ) / 100, wc_cp_price_num_decimals() );
		}
	...
	*/
	...
	public static function schedule_price_calc_tasks() {
		...
		if ( ! empty( self::$price_calc_tasks_to_queue ) ) {
			...
			Compatibility::log( 'Scheduling price calc tasks...', 'info', 'wc_cp_price_calc_tasks' );
			...
			Compatibility::log( sprintf( 'Queued %s tasks.', sizeof( self::$price_calc_tasks_to_queue ) ), 'info', 'wc_cp_price_calc_tasks' );
			...
			if ( self::$price_calc_task_runner->is_queued() && ! self::$price_calc_task_runner->is_running() ) {
				...
				Compatibility::log( 'Task runner idling. Attempting to run queued tasks manually...', 'info', 'wc_cp_price_calc_tasks' );
		...
		if ( ! empty( $filtered_component_option ) ) {
			...
			foreach ( $prices_array['price'] as $variation_id => $price ) {
				...
				if ( $priced_per_product ) {
					...
					$price                   = empty( $discount ) ? $price : round( (float) $regular_price * ( 100 - $discount ) / 100, wc_cp_price_num_decimals() );
	...
	public static function filter_show_product_get_price_html( $price_html, $product ) {
		if ( ! empty( self::$filtered_component_option ) ) {
			...
			if ( ! self::$filtered_component_option->is_priced_individually() ) {
				...
				if ( $add_suffix ) {
					$suffix     = self::$filtered_component_option->get_quantity_min() > 1 ? ' ' . __( '/ pc.', 'wpdrift-woocommerce-modules' ) : '';
	...
	*/
	...
	public static function read_price_data( $product ) {
		_deprecated_function( __METHOD__ . '()', '3.14.0', 'ProductComposite_Data_Store_CPT::read_price_data()' );
	...
	public static function get_expanded_component_options( $ids ) {
		_deprecated_function( __METHOD__ . '()', '3.14.0', 'ProductComposite_Data_Store_CPT::get_expanded_component_options()' );
	...
	public static function get_raw_component_option_prices( $ids ) {
		_deprecated_function( __METHOD__ . '()', '3.14.0', 'ProductComposite_Data_Store_CPT::get_raw_component_option_prices()' );
		$data_store = WC_Data_Store::load( 'product-composite' );
