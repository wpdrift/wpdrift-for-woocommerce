...
class Component implements ArrayAccess {
	...
	/**
	 * True if the component can have multiple options.
	...
	public function is_multiple() {
		...
		return 'yes' === $data['multiple_options'];
	}
	...
	public function get_option( $product_id ) {
		...
		if ( $product_id > 0 ) {
			if ( isset( $this->products[ $product_id ] ) ) {
				...
				$option_obj = new Product( $product_id, $this->id, $this->composite );
	...
	public function get_sorting_options() {
		...
		if ( $this->show_sorting_options() ) {
			...
			$orderby_options = apply_filters(
				...
				array(
					'default'    => __( 'Default sorting', 'wpdrift-woocommerce-modules' ),
					'popularity' => __( 'Sort by popularity', 'wpdrift-woocommerce-modules' ),
					'rating'     => __( 'Sort by average rating', 'wpdrift-woocommerce-modules' ),
					'date'       => __( 'Sort by newness', 'wpdrift-woocommerce-modules' ),
					'price'      => __( 'Sort by price: low to high', 'wpdrift-woocommerce-modules' ),
					'price-desc' => __( 'Sort by price: high to low', 'wpdrift-woocommerce-modules' ),
				),
				$this->id,
				$this->composite
			);
	...
	*/
	...
	public function get_results_per_page() {
		...
		$rpp               = in_array( $layout, array( 'single', 'progressive' ) ) ? $thumbnail_columns : $thumbnail_columns * 2;
	...
	public function get_image_size() {
		return apply_filters( 'woocommerce_composite_component_image_size', Compatibility::is_wc_version_gte( '3.3' ) ? 'woocommerce_thumbnail' : 'shop_catalog', $this );
	}
	...
	*/
	 ...
	 * Note that the query parameters are filterable - @see 'Query' class for details.
	 *
	...
	public static function query_component_options( $component_data, $query_args = array() ) {
		...
		$query = new Query( $component_data, $query_args );
	...
	public static function get_options_styles() {
		$styles = array(
			array(
				'id'          => 'thumbnails',
				'title'       => __( 'Thumbnails', 'wpdrift-woocommerce-modules' ),
				'description' => __( 'Component Options are displayed as thumbnails, paginated and arranged in columns similar to the main shop loop.', 'wpdrift-woocommerce-modules' ),
				'supports'    => array(),
			),
			array(
				'id'          => 'list',
				'title'       => __( 'List', 'wpdrift-woocommerce-modules' ),
				'description' => __( 'Component Options are displayed in a list.', 'wpdrift-woocommerce-modules' ),
				'supports'    => array(),
			),
		);
	...
	public static function get_price_display_options() {
		return array(
			array(
				'id'          => 'absolute',
				'title'       => __( 'Absolute', 'wpdrift-woocommerce-modules' ),
				'description' => sprintf( __( 'Display absolute Component Option prices, e.g. </br>%s.', 'wpdrift-woocommerce-modules' ), wc_price( 100 ) ),
			),
			array(
				'id'          => 'relative',
				'title'       => __( 'Relative', 'wpdrift-woocommerce-modules' ),
				'description' => sprintf( __( 'Display relative Component Option prices based on the price of the selected option, e.g. </br>+%1$s or -%2$s.', 'wpdrift-woocommerce-modules' ), wc_price( 100 ), wc_price( 50 ) ),
			),
			array(
				'id'          => 'hidden',
				'title'       => __( 'Hidden', 'wpdrift-woocommerce-modules' ),
				'description' => __( 'Hide Component Option prices.', 'wpdrift-woocommerce-modules' ),
			),
		);
	}
	...
	public static function get_pagination_style_options() {
		return array(
			array(
				'id'          => 'classic',
				'title'       => __( 'Classic', 'wpdrift-woocommerce-modules' ),
				'description' => __( 'Component Options are arranged in pages, similar to the main shop loop.', 'wpdrift-woocommerce-modules' ),
			),
			array(
				'id'          => 'load-more',
				'title'       => __( 'Load more', 'wpdrift-woocommerce-modules' ),
				'description' => __( 'Component Options are appended by clicking a "Load more" button.', 'wpdrift-woocommerce-modules' ),
			),
		);
	}
	...
	public static function get_select_action_options() {
		return array(
			array(
				'id'          => 'view',
				'title'       => __( 'View selection details', 'wpdrift-woocommerce-modules' ),
				'description' => __( 'Display selected product details without moving to the next Component.', 'wpdrift-woocommerce-modules' ),
			),
			array(
				'id'          => 'transition',
				'title'       => __( 'View next step', 'wpdrift-woocommerce-modules' ),
				'description' => __( 'View the next Component.', 'wpdrift-woocommerce-modules' ),
			),
		);
	}
}
