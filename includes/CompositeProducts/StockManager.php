...
class StockManager {
	...
	public function add_item( $product, $variation = false, $quantity = 1 ) {
		$this->items[] = new StockManagerItem( $product, $variation, $quantity );
	}
	 ...
	 * Note that in some cases, stock for a variation might be managed by the parent - this is tracked by the managed_by_id property in StockManagerItem.
	 *
	 * @return array
	 */
		...
		foreach ( $managed_items as $managed_item_id => $managed_item ) {
			...
			try {
				...
				if ( $product_data->is_sold_individually() && $quantity > 1 ) {
					...
					$reason = sprintf( __( 'Only 1 &quot;%s&quot; may be purchased.', 'wpdrift-woocommerce-modules' ), $product_title );
				...
				if ( false === $product_data->is_purchasable() ) {
					...
					$reason = sprintf( __( '&quot;%s&quot; cannot be purchased.', 'wpdrift-woocommerce-modules' ), $product_title );
				...
				if ( 'variation' === $product_data->get_type() ) {
					$product_title = Helpers::format_product_title( $product_title, '', wc_get_formatted_variation( $product_data, true, false ), false );
				}

				// Stock check - only check if we're managing stock and backorders are not allowed.
				if ( ! $product_data->is_in_stock() ) {

					$reason = sprintf( __( '&quot;%s&quot; is out of stock.', 'wpdrift-woocommerce-modules' ), $product_title );
					...
					$reason = sprintf( __( 'There is not enough stock of &quot;%1$s&quot; (%2$s remaining).', 'wpdrift-woocommerce-modules' ), $product_title, $product_data->get_stock_quantity() );
				...
				if ( $product_data->managing_stock() ) {
					...
					if ( isset( $quantities_in_cart[ $managed_item_id ] ) && ! $product_data->has_enough_stock( $quantities_in_cart[ $managed_item_id ] + $quantity ) ) {
						...
						$reason = sprintf( __( 'There is not enough stock of &quot;%1$s&quot; (%2$s in stock, %3$s in your cart).', 'wpdrift-woocommerce-modules' ), $product_title, $product_data->get_stock_quantity(), $quantities_in_cart[ $managed_item_id ] );
						...
						$error = sprintf( '<a href="%s" class="button wc-forward">%s</a> %s', wc_get_cart_url(), __( 'View Cart', 'wpdrift-woocommerce-modules' ), $notice );
