...
<div class="<?php echo esc_attr( implode( ' ', $gallery_classes ) ); ?>">
	...
	if ( has_post_thumbnail( $product_id ) ) {
		...
		$html .= sprintf( '<a href="%1$s" class="image zoom" title="%2$s" data-rel="%3$s">%4$s</a>', esc_url( $image_link ), esc_attr( $image_title ), esc_attr( $image_rel ), $image );
		$html .= '</figure>';

	} else {
		$html  = '<figure class="composited_product_image woocommerce-product-gallery__image--placeholder">';
		$html .= sprintf( '<a href="%1$s" class="placeholder_image zoom" data-rel="%3$s"><img class="wp-post-image" src="%1$s" alt="%2$s"/></a>', wc_placeholder_img_src(), __( 'Product placeholder image', 'wpdrift-woocommerce-modules' ), esc_attr( $image_rel ) );
		$html .= '</figure>';
	}
