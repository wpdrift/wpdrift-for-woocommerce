		...
		?><div class="component_filter cp_clearfix <?php echo 'closed' === $filter[ 'filter_toggle_state' ] ? 'closed' : 'open'; ?>" data-filter_type="<?php echo esc_attr( $filter[ 'filter_type' ] ); ?>" data-filter_id="<?php echo esc_attr( $filter[ 'filter_id' ] ); ?>" data-multiselect="<?php echo esc_attr( $filter[ 'is_multiselect' ] ) ? 'yes' : 'no'; ?>">
			<div class="component_filter_title">
				<label class="component_filter_name" >
					...
					<button class="aria_button" aria-label="<?php echo sprintf( __( 'Toggle %s', 'wpdrift-woocommerce-modules' ), $filter[ 'filter_name' ] ); ?>" aria-expanded="<?php echo 'closed' !== $filter[ 'filter_toggle_state' ] ? 'true' : 'false'; ?>"></button>
				</label>
				<a class="reset_component_filter" href="#" rel="nofollow" role="button" aria-label="<?php echo sprintf( __( 'Reset %s', 'wpdrift-woocommerce-modules' ), $filter[ 'filter_name' ] ); ?>"></a>
			</div>
				...
				?><ul class="component_filter_options"><?php
					...
					foreach ( $filter[ 'filter_options' ] as $option_id => $option_name ) {
						...
						?><li class="component_filter_option" data-option_id="<?php echo esc_attr( $option_id ); ?>">
							<a class="toggle_filter_option" href="#" rel="nofollow" role="checkbox" aria-checked="false" aria-label="<?php echo sprintf( __( 'Toggle %s', 'wpdrift-woocommerce-modules' ), $option_name ); ?>"><?php
								echo $option_name;
							?></a>
						</li><?php
					}
