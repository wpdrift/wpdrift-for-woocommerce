...
class Order {
	...
	/**
	 * Flag to short-circuit 'Order::get_order_items'.
	 * @var boolean
	 */
	public static $override_order_items_filter = false;

	/**
	 * Flag to short-circuit 'Order::get_product_from_item'.
	 *
	 * @var boolean
	 */
	...
	*/
		...
		if ( $composite && 'composite' === $composite->get_type() ) {
			...
			if ( Module::instance()->cart->validate_composite_configuration( $composite, $quantity, $configuration, 'add-to-order' ) ) {
				...
				if ( ! empty( $components ) ) {
					foreach ( $components as $component_id => $component ) {
						...
						if ( $component_option_product->needs_shipping() && $component_option->is_shipped_individually( $component_option_product ) ) {
							...
							/** Hook documented in 'Cart::set_composited_cart_item()'. */
							if ( apply_filters( 'woocommerce_composited_product_has_bundled_weight', false, $component_option_product, $component_id, $composite ) ) {
								$bundled_weight += (float) $component_option_product->get_weight( 'edit' ) * $component_quantity;
							}
						}
				...
				if ( $composite->needs_shipping() ) {
					$container_order_item->add_meta_data( '_composite_weight', (float) $composite->get_weight( 'edit' ) + $bundled_weight, true );
				}
	...
	public function get_order_items( $items, $order ) {
			...
			if ( wc_cp_is_composite_container_order_item( $item ) ) {
				...
				if ( $aggregate ) {
					...
					foreach ( $assembled_items as $child_item_id => $child_item ) {
						...
						$child              = Helpers::cache_get( 'order_item_product_' . $child_item_id );

						if ( null === $child ) {
							$child = wc_get_product( $child_id );
							Helpers::cache_set( 'order_item_product_' . $child_item_id, $child );
						}
						...
						$meta_data               = Compatibility::is_wc_version_gte( '3.1' ) ? $child_item->get_formatted_meta_data( '_', true ) : $child_item->get_formatted_meta_data();
						...
						$meta_desc_array[] = array(
							'key'   => _x( 'Qty', 'component order item qty meta key', 'wpdrift-woocommerce-modules' ),
							'value' => $child_item->get_quantity(),
						);

						if ( $sku = $child->get_sku() ) {
							$meta_desc_array[] = array(
								'key'   => _x( 'SKU', 'component order item SKU meta key', 'wpdrift-woocommerce-modules' ),
								'value' => $sku,
							);
						}
				...
				$product      = Helpers::cache_get( 'order_item_product_' . $item_id );

				if ( null === $product ) {
					$product = wc_get_product( $id );
					Helpers::cache_set( 'order_item_product_' . $item_id, $product );
				}
		...
		// Undo 'Order::get_product_from_item' short circuit.
	...
	public function get_product_from_item( $product, $item, $order = false ) {
		...
		// Undo 'Order::get_order_items' short circuit.
		self::$override_order_items_filter = $override_order_items_filter;
	...
	public function get_assembled_items( $item, $order ) {
		...
		$assembled_items = Helpers::cache_get( 'assembled_order_items_' . $item->get_id() );
		...
		foreach ( $child_items as $child_item ) {
			...
			if ( 'no' === $child_item->get_meta( '_composite_item_needs_shipping', true ) ) {
				...
				$child_product_id   = $child_item->get_product_id();
				$child_id           = $child_variation_id ? $child_variation_id : $child_product_id;
				$child_product      = Helpers::cache_get( 'order_item_product_' . $child_item_id );

				if ( null === $child_product ) {
					$child_product = wc_get_product( $child_id );
					Helpers::cache_set( 'order_item_product_' . $child_item_id, $child_product );
				}
		...
		Helpers::cache_set( 'assembled_order_items_' . $item->get_id(), $assembled_items );
