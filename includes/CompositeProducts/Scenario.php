...
class Scenario {
	...
	public function __construct( $scenario_data ) {
		...
		$this->name               = isset( $scenario_data[ 'title' ] ) ? $scenario_data[ 'title' ] : __( 'Untitled Scenario', 'wpdrift-woocommerce-modules' );
	...
	/**
	 * Returns true if the scenario contains a product/variation ID selection from a specific component. Uses 'Scenario::contains_id' but also takes exclusion rules into account.
	...
	/**
	 * Returns true if a component option ID is defined in the scenario. Also @see 'Scenario::contains_product'.
	 *
	 * @param  string  $component_id
	 * @param  int     $id
	 * @return boolean
