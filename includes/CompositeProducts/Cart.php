...
class Cart {
	 ...
	 * Main Cart instance.
	 *
	 * Ensures only one instance of Cart is loaded or can be loaded.
	 *
	 * @static
	 * @return Cart
	 * @since  1.0.0
	...
	*/
	...
	public function add_composite_to_cart( $product_id, $quantity, $configuration = array(), $cart_item_data = array() ) {
		...
		if ( $composite ) {
			...
			if ( $this->validate_composite_configuration( $composite, $quantity, $configuration ) ) {
				...
				$message = __( 'The submitted composite configuration could not be added to the cart.', 'wpdrift-woocommerce-modules' );
	 ...
	 *
	 * @param  ProductComposite  $composite      Composite product whose configuration is being parsed or generated.
	...
	public function parse_composite_configuration( $composite, $configuration = array(), $strict_mode = false ) {
		...
		foreach ( $components as $component_id => $component ) {
			...
			$component_configuration_array = isset( $configuration[ $component_id ] ) ? $configuration[ $component_id ] : array();
			$defaults                      = array(
				'product_id' => $strict_mode ? '' : $component->get_default_option(),
				'quantity'   => $component->get_quantity( 'min' ),
			);

			foreach ( $component_configuration_array as $component_configuration ) {
				$cp = wp_parse_args( $component_configuration, $defaults );

				$cp['quantity_min'] = $component->get_quantity( 'min' );
				$cp['quantity_max'] = $component->get_quantity( 'max' );
				$cp['discount']     = $component->get_discount();
				$cp['optional']     = $component->is_optional() ? 'yes' : 'no';
				$cp['static']       = $component->is_static() ? 'yes' : 'no';
				$cp['title']        = $component->get_title();
				$cp['composite_id'] = $composite->get_id();

				if ( $cp['product_id'] > 0 ) {
					$product_id = $cp['product_id'];
					// Store the product type.
					$cp['type'] = WC_Product_Factory::get_product_type( $product_id );
				}

				$parsed_configuration[ $component_id ][] = $cp;
			}
		}
	...
	public function validate_composite_configuration( $composite, $composite_quantity, $configuration, $context = '' ) {
		...
		if ( is_object( $composite ) && $composite->is_type( 'composite' ) ) {
			...
			try {
				...
				// The Stock_Manager class does exactly that.
				$composited_stock = new StockManager( $composite );
				...
				foreach ( $components as $component_id => $component ) {
					 ...
					 */
					$selected_products = empty( $configuration[ $component_id ] ) ? [] : $configuration[ $component_id ];

					foreach ( $selected_products as $product_selected ) {
						$composited_product_id = $product_selected['product_id'];
						...
						if ( ! in_array( $composited_product_id, $component->get_options() ) ) {
							if ( 'add-to-cart' === $context ) {
								$reason = sprintf( __( 'Please choose a valid &quot;%s&quot; option&hellip;', 'wpdrift-woocommerce-modules' ), $component_title );
							} else {
								$reason = sprintf( __( 'The chosen &quot;%s&quot; option is unavailable.', 'wpdrift-woocommerce-modules' ), $component_title );
							}

							if ( 'add-to-cart' === $context ) {
								$notice = sprintf( __( '&quot;%1$s&quot; cannot be added to your cart. %2$s', 'wpdrift-woocommerce-modules' ), $composite_title, $reason );
							} elseif ( 'cart' === $context ) {
								$notice = sprintf( __( '&quot;%1$s&quot; cannot be purchased. %2$s', 'wpdrift-woocommerce-modules' ), $composite_title, $reason );
							} else {
								$notice = $reason;
							}
						...
						$composited_product_validation_data = [
						...
						$composited_product_validation_data['quantity_min'] = $item_quantity_min;
						$composited_product_validation_data['quantity_max'] = $item_quantity_max;

						// Store quantity for validation.
						$item_quantity = isset( $product_selected['quantity'] ) ? absint( $product_selected['quantity'] ) : $item_quantity_min;
						$quantity      = $item_quantity * $composite_quantity;

						$component_option = $composite->get_component_option( $component_id, $composited_product_id );

						if ( ! $component_option ) {

							if ( 'add-to-cart' === $context ) {
								$reason = sprintf( __( 'Please choose another &quot;%s&quot; option&hellip;', 'wpdrift-woocommerce-modules' ), $component_title );
							} else {
								$reason = sprintf( __( 'The chosen &quot;%s&quot; option is unavailable.', 'wpdrift-woocommerce-modules' ), $component_title );
							}
						...
						$composited_product_validation_data['quantity']          = $item_quantity;
						$composited_product_validation_data['sold_individually'] = $item_sold_individually ? 'yes' : 'no';
						...
						if ( 'variable' === $composited_product_type ) {
							...
							$composited_variation_id = isset( $product_selected['variation_id'] ) ? $product_selected['variation_id'] : '';
							...
							if ( $composited_variation ) {
								...
								$composited_product_validation_data['variation_id'] = $composited_variation_id;
							}
							...
							if ( $composited_variation ) {
								...
								foreach ( $attributes as $attribute ) {
									...
									if ( isset( $product_selected['attributes'][ $taxonomy ] ) ) {

										// Get valid value from variation.
										$valid_value = isset( $variation_data[ $taxonomy ] ) ? $variation_data[ $taxonomy ] : '';

										// Allow if valid.
										if ( '' === $valid_value || $valid_value === $product_selected['attributes'][ $taxonomy ] ) {
											continue;
										}
							...
							if ( ! $all_set ) {
								...
								if ( $missing_attributes ) {
									$reason = sprintf( _n( '%1$s is a required &quot;%2$s&quot; field.', '%1$s are required &quot;%2$s&quot; fields.', sizeof( $missing_attributes ), 'wpdrift-woocommerce-modules' ), wc_format_list_of_items( $missing_attributes ), $component_title );
								} else {
									if ( 'add-to-cart' === $context ) {
										$reason = sprintf( __( 'Please choose &quot;%s&quot; options&hellip;', 'wpdrift-woocommerce-modules' ), $component_title );
									} else {
										$reason = sprintf( __( '&quot;%s&quot; is missing some required options.', 'wpdrift-woocommerce-modules' ), $component_title );
									}
								}
						...
						$validation_data[ $component_id ][] = $composited_product_validation_data;
					}
				}
				...
				if ( 'add-to-cart' === $context ) {
					$is_configuration_valid = $composited_stock->validate_stock(
						array(
							'context'         => $context,
				...
				foreach ( $validation_data as $component_id => $component_validation_data_array ) {
					foreach ( $component_validation_data_array as $component_validation_data ) {
						...
						if ( $qty < $qty_min && 'yes' !== $sold_individually ) {
							...
							$reason = sprintf( __( 'The quantity of &quot;%1$s&quot; cannot be lower than %2$d.', 'wpdrift-woocommerce-modules' ), $component_validation_data['title'], $qty_min );
							...
							$reason = sprintf( __( 'The quantity of &quot;%1$s&quot; cannot be higher than %2$d.', 'wpdrift-woocommerce-modules' ), $component_validation_data['title'], $qty_max );
				...
				if ( has_filter( $validation_filter_name ) ) {
					 ...
					 * @param  string               $composite_id
					 * @param  Stock_Manager  $composited_stock
		...
		// return [ $is_configuration_valid, $validation_data, $composite_configuration ];
	...
	private function set_composited_cart_item( $cart_item, $composite ) {
		...
		if ( $cart_item['data']->needs_shipping() ) {
			...
			if ( $component_option && false === $component_option->is_shipped_individually() ) {
				 ...
				 * @param   string                $component_id
				 * @param   ProductComposite  $composite_product
				 */
	...
	*/
	...
	public function update_composite_cart_redirect_message( $message ) {
		return __( 'Cart updated.', 'wpdrift-woocommerce-modules' );
	}
	...
	public function add_to_cart_validation( $add, $product_id, $quantity, $variation_id = '', $variations = array(), $cart_item_data = array() ) {
		...
		if ( 'composite' === $product_type ) {
			...
			if ( ! is_object( $composite ) || ! is_a( $composite, __NAMESPACE__ . '\ProductComposite' ) ) {
				return false;
			}
	...
	public function add_cart_item_data( $cart_item_data, $product_id ) {
		...
		if ( 'composite' === $product_type ) {
			...
			if ( ! isset( $cart_item_data['composite_data'] ) ) {
				...
				if ( false === Compatibility::is_wc_version_gte( '3.5' ) && false === $updating_composite_in_cart && ( $composite = wc_get_product( $product_id ) ) && $composite->is_sold_individually() ) {
					foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
						if ( $product_id === $cart_item['product_id'] && 'product' === $composite->get_sold_individually_context() ) {
							throw new Exception( sprintf( '<a href="%s" class="button wc-forward">%s</a> %s', wc_get_cart_url(), __( 'View Cart', 'wpdrift-woocommerce-modules' ), sprintf( __( 'You cannot add another &quot;%s&quot; to your cart.', 'wpdrift-woocommerce-modules' ), $composite->get_title() ) ) );
						} elseif ( wc_cp_is_composite_container_cart_item( $cart_item ) && $configuration === $cart_item['composite_data'] ) {
							throw new Exception( sprintf( '<a href="%s" class="button wc-forward">%s</a> %s', wc_get_cart_url(), __( 'View Cart', 'wpdrift-woocommerce-modules' ), sprintf( __( 'You have already added an identical &quot;%s&quot; to your cart. You cannot add another one.', 'wpdrift-woocommerce-modules' ), $composite->get_title() ) ) );
						}
					}
				}
			}
	...
	public function add_items_to_cart( $composite_cart_key, $composite_id, $composite_quantity, $variation_id, $variation, $cart_item_data ) {
		...
		if ( wc_cp_is_composite_container_cart_item( $cart_item_data ) ) {
			...
			$composited_cart_data = array(
				'composite_parent' => $composite_cart_key,
				'composite_data'   => $cart_item_data['composite_data'],
			);

			// Now add all items - yay!
			foreach ( $cart_item_data['composite_data'] as $component_id => $component_configuration_array ) {
				foreach ( $component_configuration_array as $component_configuration ) {
					 ...
					 *
					 * @hooked Addons_Compatibility::before_composited_add_to_cart()
					 */
					 ...
					 * @param  array   $composited_item_cart_data
					 *
					 * @hooked Addons_Compatibility::after_composited_add_to_cart()
					 */
		...
		if ( ! empty( $cart_contents ) ) {
			...
			foreach ( $cart_contents as $cart_item_key => $cart_item_values ) {
				if ( wc_cp_maybe_is_composited_cart_item( $cart_item_values ) ) {
					...
					} elseif ( isset( $cart_item_values['composite_item'] ) && $container_item['data']->is_type( 'composite' ) && ! $container_item['data']->has_component( $cart_item_values['composite_item'] ) ) {
						unset( WC()->cart->cart_contents[ $cart_item_key ] );
					}
				}
			}
			...
			if ( ! empty( $composite_children ) ) {
				...
				foreach ( $composite_children as $child_key => $child_item ) {
					...
					if ( $child_item['data']->is_sold_individually() && $quantity > 0 ) {
						...
						$child_item_product_index = array_search( $child_item_product_id, array_column( $child_item['composite_data'], 'product_id' ) );
						$child_quantity           = $child_item['composite_data'][ $child_item_id ][ $child_item_product_index  ]['quantity'];
	...
	public function update_cart_validation( $passed, $cart_item_key, $cart_item, $quantity ) {
		...
		if ( $parent = wc_cp_get_composited_cart_item_container( $cart_item ) ) {
			...
			if ( $quantity < $min_quantity ) {
				...
				wc_add_notice( sprintf( __( 'The quantity of &quot;%1$s&quot; cannot be lower than %2$d.', 'wpdrift-woocommerce-modules' ), $cart_item['data']->get_title(), $min_quantity ), 'error' );
				return false;

			} elseif ( $max_quantity && $quantity > $max_quantity ) {

				wc_add_notice( sprintf( __( 'The quantity of &quot;%1$s&quot; cannot be higher than %2$d.', 'wpdrift-woocommerce-modules' ), $cart_item['data']->get_title(), $max_quantity ), 'error' );
				return false;

			} elseif ( $quantity % $parent_quantity != 0 ) {

				wc_add_notice( sprintf( __( 'The quantity of &quot;%1$s&quot; must be entered in multiples of %2$d.', 'wpdrift-woocommerce-modules' ), $cart_item['data']->get_title(), $parent_quantity ), 'error' );
				return false;
						...
						if ( wc_cp_is_composite_container_cart_item( $cart_item_data ) ) {
							...
							if ( $composite->needs_shipping() ) {
								...
								if ( $bundled_value > 0 ) {
									...
									$composite->set_price( (float) $composite_price + $bundled_value / $composite_qty );
								}
								...
								if ( $bundled_weight > 0 ) {
									...
									$composite->set_weight( (float) $composite_weight + $bundled_weight / $composite_qty );
								}
	...
	public function sold_individually_found_in_cart( $found, $product_id, $variation_id, $cart_item ) {
		...
		foreach ( WC()->cart->get_cart() as $search_cart_item ) {
			if ( $product_id === $search_cart_item['product_id'] && 'product' === $product->get_sold_individually_context() ) {
				...
				throw new Exception( sprintf( '<a href="%s" class="button wc-forward">%s</a> %s', wc_get_cart_url(), __( 'View Cart', 'wpdrift-woocommerce-modules' ), sprintf( __( 'You have already added an identical &quot;%s&quot; to your cart. You cannot add another one.', 'wpdrift-woocommerce-modules' ), $product->get_title() ) ) );
			}
		}
