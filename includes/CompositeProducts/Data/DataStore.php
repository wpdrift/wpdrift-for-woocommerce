 ...
 */
class DataStore extends WC_Product_Data_Store_CPT {
	...
	protected function handle_updated_props( &$product ) {
		...
		if ( Compatibility::is_wc_version_gte( '3.6' ) ) {
	...
	public function read_price_data( &$product, $task_runner_args = false ) {
		...
		if ( ! $price_data && false === $task_runner_args ) {
			$price_data = Helpers::cache_get( 'price_data_' . $product->get_id() );
		}

		$price_data_hash = apply_filters(
			'woocommerce_composite_price_data_hash',
			md5(
				json_encode(
					array(
						$max_iterations,
		...
		if ( $resume_from_permutation ) {
			...
			if ( false === $task_runner_args ) {
				...
				Products::schedule_price_calc_task(
					array(
						'min'          => $price_data['min'],
						'max'          => $price_data['max'],
		...
		Helpers::cache_set( 'price_data_' . $product->get_id(), $price_data );
	...
	private function get_permutation_price( $permutation, $components, $prices, $min_or_max = 'min' ) {
		...
		foreach ( $components as $component_id => $component ) {
			...
			if ( 'max' !== $min_or_max ) {
				...
				$price += $quantity * (float) $component_option_price;
	...
	public function get_raw_component_option_prices( $product_ids ) {
		...
		$results           = Helpers::cache_get( $results_cache_key );
		...
		if ( null === $results ) {
			...
			Helpers::cache_set( $results_cache_key, $results );
		}
		...
		foreach ( $results as $result ) {
			...
			$price_min = isset( $prices['min'][ $product_id ] ) ? min( (float) $result['price'], $prices['min'][ $product_id ] ) : (float) $result['price'];
			$price_max = isset( $prices['max'][ $product_id ] ) ? max( (float) $result['price'], $prices['max'][ $product_id ] ) : (float) $result['price'];
	...
	public static function get_expanded_component_options( $product_ids, $return = 'expanded' ) {
		...
		$cached_results    = Helpers::cache_get( $results_cache_key );
		...
		if ( null === $cached_results ) {
			...
			Helpers::cache_set( $results_cache_key, $cached_results );
		}
	...
	public function preload_component_options_data( $product_ids ) {
		...
		$data      = Helpers::cache_get( $cache_key );
		...
		if ( null === $data ) {
			...
			Helpers::cache_set( $cache_key, $data );
		}
	}
	...
	public function query_component_options( $component_data, $query_args ) {
		...
		if ( false === $query_args['orderby'] && false === $query_args['per_page'] ) {
			...
			if ( false === $query_args['disable_cache'] && ! empty( $component_data['component_id'] ) && ! empty( $component_data['composite_id'] ) && ! defined( 'DEBUG_QUERY_TRANSIENTS' ) ) {
