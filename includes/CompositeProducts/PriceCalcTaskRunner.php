 ...
 */
class PriceCalcTaskRunner extends WP_Background_Process {
	...
	public function dispatch() {
		...
		if ( is_wp_error( $dispatched ) ) {
			Compatibility::log( sprintf( 'Unable to dispatch task runner: %s', $dispatched->get_error_message() ), 'error', 'wc_cp_price_calc_tasks' );
		}
	...
	protected function task( $data ) {
		...
		if ( empty( $data ) || empty( $data['composite_id'] ) || empty( $data['resume'] ) || empty( $data['hash'] ) ) {
			...
			Compatibility::log( 'Invalid task data.', 'info', 'wc_cp_price_calc_tasks' );
			return false;
		}
		...
		Compatibility::log( sprintf( 'Calculating min/max catalog price of composite product #%s. Resuming from permutation %s. Iteration %s.', absint( $data['composite_id'] ), $resume_from, $iteration ), 'info', 'wc_cp_price_calc_tasks' );

		if ( false === ( $product = wc_get_product( $data['composite_id'] ) ) || ! $product->is_type( 'composite' ) ) {

			Compatibility::log( 'Invalid product ID. Task aborted.', 'info', 'wc_cp_price_calc_tasks' );
			return false;
		}

		$price_data = $product->get_data_store()->read_price_data(
			$product,
			array(
		...
		if ( $stored_data = $this->get_task_data( $this->get_task_key( $data ) ) ) {
			...
			if ( $price_data['hash'] !== $hash || $price_data['hash'] !== $stored_data['hash'] ) {
				...
				Compatibility::log( 'Τask hash changed: Starting over!', 'info', 'wc_cp_price_calc_tasks' );
		...
		if ( isset( $price_data['resume'] ) ) {
			...
			Compatibility::log( 'Re-scheduling task...', 'info', 'wc_cp_price_calc_tasks' );
			...
			Compatibility::log( 'Max allowed iterations count exceeded. Task aborted.', 'info', 'wc_cp_price_calc_tasks' );
			return false;
		}

		Compatibility::log( 'Task complete.', 'info', 'wc_cp_price_calc_tasks' );
		return false;
	}
}
